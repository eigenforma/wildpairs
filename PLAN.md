...
| phase | what | where | state |
|---|---|---|---|
| E2-P0 | Freeze CondaQA snapshot (HF `lasha-nlp/CONDAQA`); measure the single-sentence-edit yield rate (pre-registered fallback target ≥400 anchors); payload-Jaccard per pair per edit type | this box | next after E1-P0 |
| E2-P1 | Acquire + freeze hosts: enwiki-20210701 (archive.org, ~19 GB → Agora NVMe), PMC-OA CC-BY slice (AWS mirror), RFC tarball (shared with E1) | Agora (storage + 300 Mbps egress budget) | RUNBOOK R5 |
