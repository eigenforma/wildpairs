...
| E2-P1 | Acquire + freeze hosts: enwiki-20210701 (archive.org, ~19 GB), PMC-OA CC-BY slice (AWS mirror), RFC text corpus (shared with E1) — **storage: Agora `/mnt/coldstore` (1 TB volume the operator provisioned 2026-08-05; corpora on the mining box, weights on Forge's vault; Forge reads corpora via NFS-over-DAC at sweep time)** | Agora coldstore, 300 Mbps egress budget | **IN PROGRESS 2026-08-05** — rfc-text complete (550 MB); enwiki downloading (resumable, background); PMC deferred to `prereg-e2` slice spec; sha256 freeze + external-corpus manifest entries when transfers finish |
