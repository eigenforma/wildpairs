...
**Parity-gate lesson (2026-08-06, from the H4 GPU shard):** parity is measured against **the pinned path's actual numerics, not an idealized fp32**. Two checkpoints (gte-base, mxbai-embed-large) are fp16-native and the pinned Wu-CPU path computes them in fp16; GPU substitution failed the 1e-5 gate in both fp16 (accumulation order) and forced-fp32 (2^-10 quantum offset vs the fp16 reference) modes. Ruling: **those two configs are CPU-only**; the other seven passed at ~1e-7 and may run on GPU. The gate caught a real substitution error twice before any number was admitted — this is the gate working.
