...
Control: **SSH from Wu is passwordless to forge and agora** (`ssh scott@10.1.20.223`, `ssh scott@10.1.20.207`) — verified. Fleet convention (`sync_node.sh`): nodes are pull-consumers, never push; Wu orchestrates via `deploy_secrets.ps1`. Known nit: `/forge/status` GPU telemetry fields are null while `nvidia-smi` works over SSH. Project_Intern is a **parked** effort (operator ruling 2026-08-05) — we reuse its machinery but do not develop it; wildpairs will carry its own thermal watch (`scripts/fleet_watch.py`, a ~20-line ssh+nvidia-smi loop) before any overnight run.
