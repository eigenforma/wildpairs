"""Annotator B: the local gpt-oss-120b (different model family, never exposed to any
...
PROTOCOL = (ROOT / "docs" / "annotation_protocol_bis.md").read_text(encoding="utf-8")
PACKET = json.loads((ROOT / "results" / "verification" / "bis_audit_blind_packet.json").read_text(encoding="utf-8"))

# The harmony chat template fails to parse JSON braces in the prompt (server 500 at render),
# so the protocol's Output-format section is stripped and the verdict is one bare word.
...
def main():
    ...
    for it in PACKET:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] item {i}: {v} ({time.time()-t0:.0f}s) [{len(done)}/200]", flush=True)
