"""Annotator B for the E1-P5 errata label-noise arm (amendment A5): the local gpt-oss-120b
...
PACKET = json.loads((ROOT / "results" / "verification" / "errata_audit_blind_packet.json").read_text(encoding="utf-8"))
...
def main():
    ...
    for it in PACKET:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] item {i}: {v} ({time.time()-t0:.0f}s) [{len(done)}/300]", flush=True)
